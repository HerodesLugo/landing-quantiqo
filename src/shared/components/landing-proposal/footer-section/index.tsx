...
const HoverGlass = dynamic(() => import("../../threeJs/HoverGlass"), {
  ssr: false,
  loading: () => null,
});
...
const Footer = ({ isActive, className }: FooterProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative  flex h-full flex-col xl:items-center xl:justify-between">
        <ButtonCTA />

        <div className="flex w-full justify-between xl:mt-10  max-xl:flex-col">
          {/* Usamos la misma clase anim-middle para ambos lados para que suban coordinados */}

          <div className="flex flex-col sm:gap-5 anim-middle will-change-transform max-xl:mb-6">
            <Image
            
              src="/images/footer/quantiqo_tm.png"
              alt="quantiqo_tm"
              className="h-16 max-w-xs object-contain max-xl:w-[9.5625rem]"
              height={70}
              width={368}
            />
          ...
          <div className="flex w-full flex-col items-start xl:items-center pt-3  ">
            <div className="flex flex-col items-start xl:items-center">
              <h2 className="xl:justify-start  max-xl:text-left tracking-[5px] text-primary-100 text-4xl max-xl:text-2xl text-center max-xl:mb-3">
                <span className="font-light">WE IMPULSE </span>
                <span className="font-medium">SUCCESS STORIES</span>
                <span className="font-light"> HAPPEN.</span>
              </h2>

              <div className=" w-full max-w-[776px] mix-blend-overlay  text-white text-2xl font-normal  tracking-[4.80px] max-xl:text-lg max-sm:text-sm">
        ...
        {/* 3. BOTTOM ROW */}
        <div className="flex w-full xl:gap-20  relative z-20 xl:pr-20 items-start xl:items-end justify-between mt-10 max-xl:flex-col  max-xl:pr-20 max-sm:pr-0">
          <div className="max-sm:w-full max-sm:hidden max-xl:mb-10">
            <NavItems />
          </div>

          <div className=" items-start w-full justify-end xl:gap-16 max-xl:gap-6 flex max-xl:flex-col anim-bottom will-change-transform ">
            {LOCATIONS.map((loc) => (
              <div
                key={loc.title}
                className="inline-flex flex-col w-full  items-start justify-start sm:gap-6 max-xl:gap-2"
              >
                <h3 className="self-stretch justify-start text-2xl font-semibold tracking-[3.20px] text-white max-xl:text-lg">
                  {loc.title}
                </h3>
                <div className="h-px w-full bg-white" />
                <div className="flex flex-col items-start justify-start gap-2 w-full ">
                  <address className="xl:w-72 justify-start text-sm font-normal uppercase not-italic text-neutral-400 leading-relaxed w-full">
                    {loc.content}
                  </address>
                </div>
              </div>
